...
OPENDISTRO_DN="/C=IT/ST=Italy/L=Rome/O=Team-ACD"   # Edit here and in opensearch.yml

mkdir -p certs/{ca,os-dashboards}

# Root CA
openssl genrsa -out certs/ca/ca.key 2048
openssl req -new -x509 -sha256 -days 1095 -subj "$OPENDISTRO_DN/CN=CA-OpenSearch" -key certs/ca/ca.key -out certs/ca/ca.pem
